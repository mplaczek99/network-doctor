...
func TestToolsForDefinitions(t *testing.T) {
	...
	nmapArgs := []string{"-sT", "-T4", "-Pn", "--host-timeout", "110s", "github.com"}
	wantHost := []want{
...
func TestNmapTool(t *testing.T) {
	...
	want := []string{"-sT", "-T4", "-Pn", "--host-timeout", "110s", "-p", "8443", "example.com"}
	...
	wantAll := []string{"-sT", "-T4", "-Pn", "--host-timeout", "110s", "example.com"}
	...
	wantV6 := []string{"-sT", "-T4", "-Pn", "--host-timeout", "110s", "-6", "-p", "8443", "2001:db8::1"}
